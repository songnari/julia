...
"""
    checkout_index(repo::GitRepo, idx::Option{GitIndex} = null; options::CheckoutOptions = CheckoutOptions())
...
"""
function checkout_index(repo::GitRepo, idx::Option{GitIndex} = null;
                        options::CheckoutOptions = CheckoutOptions())
    @check ccall((:git_checkout_index, :libgit2), Cint,
                 (Ptr{Void}, Ptr{Void}, Ptr{CheckoutOptions}),
                 repo.ptr,
                 isnull(idx) ? C_NULL : unwrap(idx).ptr,
                 Ref(options))
end
...
function reset!(repo::GitRepo, obj::Option{<:GitObject}, pathspecs::AbstractString...)
    @check ccall((:git_reset_default, :libgit2), Cint,
                 (Ptr{Void}, Ptr{Void}, Ptr{StrArrayStruct}),
                 repo.ptr,
                 isnull(obj) ? C_NULL : unwrap(obj).ptr,
                 collect(pathspecs))
