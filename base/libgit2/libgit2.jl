...
The keyword arguments are:
  ...
  * `payload::Option{<:AbstractCredentials}=null`: provides credentials, if necessary,
...
function fetch(repo::GitRepo; remote::AbstractString="origin",
               ...
               payload::Option{<:AbstractCredentials}=null)
...
The keyword arguments are:
  ...
  * `payloadOption{<:AbstractCredentials}=null`: provides credentials, if necessary,
    for instance if `remote` is a private repository.
...
function branch!(repo::GitRepo, branch_name::AbstractString,
    ...
    if isnull(branch_ref)
        ...
        commit_id = if isempty(commit)
            if isnull(branch_rmt_ref)
                ...
                tmpcmt = with(peel(GitCommit, unwrap(branch_rmt_ref))) do hrc
                    GitHash(hrc)
                end
                close(unwrap(branch_rmt_ref))
                tmpcmt
            end
        else
            GitHash(commit)
    ...
    try
        ...
        if !isempty(track) # setup tracking
            try
                with(GitConfig, repo) do cfg
                    ...
                    set!(cfg, "branch.$branch_name.merge", name(unwrap(branch_ref)))
                end
            catch
        ...
        if set_head
            ...
            with(peel(GitTree, unwrap(branch_ref))) do btree
                checkout_tree(repo, btree)
            end

            # switch head to the branch
            head!(repo, unwrap(branch_ref))
        end
    finally
        close(unwrap(branch_ref))
    end
    return
end
        ...
        if !isempty(branch) # merge provided branch into HEAD
            ...
            if isorphan(repo)
                ...
                with(head(repo)) do head_ref
                    ...
                    try
                        [GitAnnotated(repo, unwrap(tr_brn_ref))]
                    finally
                        close(unwrap(tr_brn_ref))
                    end
                end
            end
        end
...
function rebase!(repo::GitRepo, upstream::AbstractString="", newbase::AbstractString="")
    with(head(repo)) do head_ref
        ...
        upst_ann = if isempty(upstream)
            ...
            try
                GitAnnotated(repo, unwrap(brn_ref))
            finally
                close(brn_ref)
            end
        else
        ...
        try
            ...
            try
                ...
                #!isnull(onto_ann) && close(unwrap(onto_ann))
                close(sig)
            end
        finally
            if !isempty(newbase)
                close(unwrap(onto_ann))
            end
            close(upst_ann)
            close(head_ann)
        end
