...
"""
    validargs(::Type{<:TimeType}, args...) -> Option{ArgumentError}

Determine whether the given arguments consitute valid inputs for the given type.
Returns a `Option{ArgumentError}` where [`null`](@ref) signifies success.
"""
function validargs end

"""
    argerror([msg]) -> Option{ArgumentError}

Construct an `Option{ArgumentError}` with the given message, or [`null`](@ref) if no message
